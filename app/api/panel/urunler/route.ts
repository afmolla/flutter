...
const VALID = new Set(URUN_KATEGORILER.map((k) => k.id));
...
export async function POST(req: Request) {
  return withSiteFromRequest(req, async () => {
    ...
    const validKat = !!kategoriId && VALID.has(kategoriId);
    if (!baslik || !ozet || !validKat || !imageSrc) {
    ...
    const kayit = await urunEkle({
      ...
      varyantlar: Array.isArray(body.varyantlar) ? body.varyantlar : [],
