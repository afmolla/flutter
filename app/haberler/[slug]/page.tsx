...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const h = await haberBySlug(slug);
  if (!h || !h.yayin) return {};
  const base = await siteUrl();
  return {
    title: h.baslik,
    description: h.aciklama,
    alternates: { canonical: `${base.replace(/\/$/, "")}/haberler/${slug}` },
...
export default async function HaberDetayPage({ params }: Props) {
  ...
  const jsonLd = {
    ...
    publisher: { "@type": "Organization", name: "Ayfleks Ambalaj" },
  };

  return (
    <AyfleksShell inside>
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd) }} />
      <AyfleksPageHero title={h.baslik} crumbs={[{ label: "Anasayfa", href: "/" }, { label: "Haberler", href: "/haberler" }, { label: h.baslik }]} />
