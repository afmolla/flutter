import { AYFLEKS_SUBDIR, dataSubdirForPrefix, isPortfolioPath, portfolioPrefixes } from "@/lib/site-config";
...
export function detectSiteFromPathname(pathname: string): DetectedSite {
  ...
  return { prefix: MOLLA_SITE_PREFIX_SENTINEL, subdir: AYFLEKS_SUBDIR };
}

function subdirToCookieName(subdir: string): string {
  const s = (subdir || AYFLEKS_SUBDIR).trim().replace(/[^a-z0-9_-]/gi, "_") || AYFLEKS_SUBDIR;
...
export function detectSiteFromCookies(cookieHeader: string, hintPath?: string): DetectedSite | null {
  ...
  const ayfleks = trySubdir(AYFLEKS_SUBDIR, MOLLA_SITE_PREFIX_SENTINEL);
  if (ayfleks) return ayfleks;
...
export function detectSiteFromRequestParts(parts: {
  ...
  if (path.startsWith("/api")) {
    ...
    if (fromPath.subdir !== AYFLEKS_SUBDIR || path === "/" || path.startsWith("/panel")) {
      return fromPath;
    }
  }
