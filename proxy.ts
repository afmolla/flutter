...
/** Ayfleks kök sitesinde erişilebilir rotalar (portföy iç rotası olarak işaretli olsa da) */
function isAyfleksPublicRoute(pathname: string): boolean {
  const p = (pathname.split("?")[0] || "/").replace(/\/+$/, "") || "/";
  if (p.startsWith("/p/")) return true;
  if (p === "/urunler" || p.startsWith("/urun/")) return true;
  if (p === "/iletisim" || p === "/randevu") return true;
...
export function proxy(req: NextRequest) {
  ...
  if (!matched) {
    if (isPortfolioInternalRoute(pathname)) {
      if (!isActivePortfolioRewrite(req) && !isAyfleksPublicRoute(pathname)) {
        const u = req.nextUrl.clone();
        u.pathname = "/";
        return NextResponse.redirect(u, 308);
      }
      const reqHeaders = requestHeadersWithSite(req, MOLLA_SITE_PREFIX_SENTINEL, "ayfleks");
      return NextResponse.next({ request: { headers: reqHeaders } });
    }
    const reqHeaders = requestHeadersWithSite(req, MOLLA_SITE_PREFIX_SENTINEL, "ayfleks");
    return NextResponse.next({ request: { headers: reqHeaders } });
